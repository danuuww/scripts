...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromHex("6494ff"),
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        -- BluuHub row model: one vertical ScrollingFrame. Side 1/2 share a pair
        -- row (each column 50%), Side 3 closes the current pair and takes a
        -- full row. Creation order equals visual order.
        local CurrentPairRow
        local CurrentPairLeft
        local CurrentPairRight
        local CurrentPairBoxes
        local RowLayoutOrder = 0

        -- Forward-declared so row helpers can register pairs on the tab before
        -- the Tab table literal is assigned below.
        local Tab
        ...
        local function EnsurePairRow()
            if CurrentPairRow and CurrentPairRow.Parent then
                return CurrentPairRow
            end
            RowLayoutOrder += 1
            CurrentPairRow = New("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 0),
                AutomaticSize = Enum.AutomaticSize.Y,
                LayoutOrder = RowLayoutOrder,
                Parent = TabContainer,
            })
            New("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                Padding = UDim.new(0, 6),
                SortOrder = Enum.SortOrder.LayoutOrder,
                Parent = CurrentPairRow,
            })
            CurrentPairLeft = CreateColumn(CurrentPairRow, 1)
            CurrentPairRight = CreateColumn(CurrentPairRow, 2)
            CurrentPairBoxes = { Left = {}, Right = {} }
            if Tab and Tab.AllPairBoxes then
                table.insert(Tab.AllPairBoxes, CurrentPairBoxes)
            end
            return CurrentPairRow
        end

        local function NewBothSlot()
            CurrentPairRow = nil
            CurrentPairLeft = nil
            CurrentPairRight = nil
            CurrentPairBoxes = nil
            RowLayoutOrder += 1
            return New("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 0),
        ...
        local function SyncPairHeights(Boxes)
            ...
            local function ColumnNaturalHeight(SideKey)
                local Height = 0
            ...
            local LeftNatural = ColumnNaturalHeight("Left")
            local RightNatural = ColumnNaturalHeight("Right")
            if LeftNatural <= 0 and RightNatural <= 0 then
                return
            end
            ...
            ApplyColumn("Left", LeftNatural)
            ApplyColumn("Right", RightNatural)
        end


        local function GetSideParent(Side: number)
            if Side == 3 then
                return NewBothSlot()
            end

            EnsurePairRow()
            return if Side == 1 then CurrentPairLeft else CurrentPairRight
        end
        ...
        do
            ...
            --// Tab Container \\--
            -- BluuHub row model: single ScrollingFrame, flows left/right pairs and
            -- full-width ("both") groupboxes in creation order via UIListLayout.
        ...
        --// Tab Table \\--
        -- (assigns the forward-declared Tab above; keep it that way so the
        -- row helpers can see the same variable)
        Tab = {
            Name = Name,
            Description = Description,
        ...
        function Tab:RefreshSides()
            -- BluuHub row model: rows live inside the TabContainer ScrollingFrame
            -- via UIListLayout, so no manual side positioning is needed.
        end
        ...
        local function AddTabbox(self, Info)
            ...
            local BoxHolder = New("Frame", {
                ...
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 0),
                Parent = if ParentObj.Type == "Groupbox" then ParentObj.Container else GetSideParent(Info.Side),
            })
            New("UIListLayout", {
                Padding = UDim.new(0, 6),
                Parent = BoxHolder,
        ...
        function Tab:AddGroupbox(Info)
            ...
            local GroupboxParent

            if Info.Side == 3 then
                GroupboxParent = NewBothSlot()
            elseif Info.Side == 1 then
                EnsurePairRow()
                GroupboxParent = CurrentPairLeft
            else
                EnsurePairRow()
                GroupboxParent = CurrentPairRight
            end
            ...
            function Groupbox:Resize()
                ...
                if Groupbox.SyncPairHeights then
                    local SyncFn = Groupbox.SyncPairHeights
                    task.delay(0.25, function()
                        if not Library.Unloaded and Groupbox.SyncPairHeights == SyncFn then
                            SyncFn()
                        end
                    end)
                end
            ...
            if CurrentPairBoxes and Info.Side ~= 3 then
                local PairBoxes = CurrentPairBoxes
                table.insert(PairBoxes[SideKey], Groupbox)
                Groupbox.SyncPairHeights = function()
        ...
        function Tab:Show()
            ...
            if Tab.AllPairBoxes then
                for _, PairBoxes in Tab.AllPairBoxes do
                    SyncPairHeights(PairBoxes)
                end
            end
